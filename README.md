...
* `notebooks/Triangle_GHz_like_photonic_states.ipynb` — Triangle (3-mode) SPDC network → W/GHZ-like post-selected states.
...
1. [Overview of the optimization pipeline](#1-overview-of-the-optimization-pipeline)
2. [Precise experiment descriptions (initial & target states) for each notebook](#2-experiments-initial--target-states)
3. [QUBO formulation — general recipe and specific choices used in the notebooks](#3-qubo-formulation--from-scratch)
4. [Continuous optimization — objective, gradients, parameterizations, and practical choices](#4-continuous-optimization--objective--gradients)
...
### A. Triangle photonic experiment

**Notebook:** `Triangle_GHz_like_photonic_states.ipynb`
...
* Single-pair (low pump) approximation: the two-photon subspace is spanned by $|110\rangle$, $|101\rangle$, $|011\rangle$ where $|110\rangle = a^\dagger b^\dagger|0\rangle$.
...
* BS A–B with angle $\theta$: creation ops transform by 
...
### B. Linear 4-mode cluster

**Notebook:** `Linear_4_node_cluster.ipynb`
...
### C. Star-shaped repeater node

**Notebook:** `Star_shaped_photonic_network.ipynb`

**Physical setup (initial):**

* Four leaves (L0, L1, L2, L3) representing outgoing channels; pairwise SPDC edges among the four leaves (complete graph of 4 nodes → 6 edges).
...
* **T1** — *Pair-Bell style:* concentrate amplitude on matching $M_1$ = (L0,L1) and (L2,L3): target vector $\mathbf{t}_1 = (1,0,0)$
...
2. **Off-diagonal $Q_{ij}$** — pairwise interaction:
   ...
   * **Synergy (encourage pair)**: if $i$ and $j$ together enable desirable interference (e.g., two edges that form a perfect matching), set $Q_{ij} = -\gamma$
   * **Conflict (penalize pair)**: if $i$ and $j$ are incompatible (share the same physical port or violate hardware constraints), set $Q_{ij} = +\delta$
...
**Acknowledgements / inspiration:** Ideas and graph-based formulation follow recent works leveraging graph/network representations for photonic experiment design (Prof. Xuemei Gu and collaborators).
